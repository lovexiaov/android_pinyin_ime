...
public class BalloonHint extends PopupWindow {
   ...
   * specification.
   */
  public void setBalloonConfig(String label, float textSize, boolean textBold, int textColor,
  ...
  public void delayedShow(long delay, int locationInParent[]) {
    ...
    if (delay <= 0) {
      ...
      showAtLocation(mParent, Gravity.LEFT | Gravity.TOP, locationInParent[0],
          locationInParent[1] + mParentLocationInWindow[1]);
    } else {
      mBalloonTimer.startTimer(delay, BalloonTimer.ACTION_SHOW, locationInParent, -1, -1);
    }
  }
  ...
  public void delayedUpdate(long delay, int locationInParent[], int width, int height) {
    ...
    if (delay <= 0) {
      ...
      mBalloonTimer.startTimer(delay, BalloonTimer.ACTION_UPDATE, locationInParent, width, height);
    }
  }
  ...
  public void delayedDismiss(long delay) {
    ...
    if (delay <= 0) {
      ...
      mBalloonTimer.startTimer(delay, BalloonTimer.ACTION_HIDE, null, -1, -1);
    }
  }
  ...
  private void setBalloonSize(int width, int height) {
    int widthMeasureSpec = MeasureSpec.makeMeasureSpec(width, mMeasureSpecMode);
    int heightMeasureSpec = MeasureSpec.makeMeasureSpec(height, mMeasureSpecMode);
  ...
  private class BalloonTimer extends Handler implements Runnable {
    ...
    public void run() {
      switch (mAction) {
        case ACTION_SHOW:
          mParent.getLocationInWindow(mParentLocationInWindow);
          showAtLocation(mParent, Gravity.LEFT | Gravity.TOP, mPositionInParent[0],
        ...
        case ACTION_UPDATE:
          mParent.getLocationInWindow(mParentLocationInWindow);
          update(mPositionInParent[0], mPositionInParent[1] + mParentLocationInWindow[1], mWidth,
              mHeight);
      }
      mTimerPending = false;
    }
  ...
  private class BalloonView extends View {
    ...
    @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
      ...
      int maxWidth = Environment.getInstance().getScreenWidth() - mPaddingLeft - mPaddingRight;
    ...
    @Override protected void onDraw(Canvas canvas) {
      ...
      if (null != mIcon) {
        ...
        mIcon.setBounds(marginLeft, marginTop, width - marginRight, height - marginBottom);
        mIcon.draw(canvas);
      } else if (null != mLabel) {
        ...
        if (x < mPaddingLeft) {
          x = mPaddingLeft;
          labelToDraw = getLimitedLabelForDrawing(mLabel, width - mPaddingLeft - mPaddingRight);
        }
