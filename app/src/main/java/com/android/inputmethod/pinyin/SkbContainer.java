...
public class SkbContainer extends RelativeLayout implements OnTouchListener {
  ...
  public void toggleCandidateMode(boolean candidatesShowing) {
    if (null == mMajorView
        || !mInputModeSwitcher.isChineseText()
  ...
  private void updateSkbLayout() {
    ...
    switch (mSkbLayout) {
      case R.xml.skb_qwerty:
        majorSkb =
            skbPool.getSoftKeyboard(R.xml.skb_qwerty, R.xml.skb_qwerty, screenWidth, skbHeight,
                mContext);
        break;

      case R.xml.skb_sym1:
        majorSkb = skbPool.getSoftKeyboard(R.xml.skb_sym1, R.xml.skb_sym1, screenWidth, skbHeight,
            mContext);
        break;

      case R.xml.skb_sym2:
        majorSkb = skbPool.getSoftKeyboard(R.xml.skb_sym2, R.xml.skb_sym2, screenWidth, skbHeight,
            mContext);
        break;

      case R.xml.skb_smiley:
        majorSkb =
            skbPool.getSoftKeyboard(R.xml.skb_smiley, R.xml.skb_smiley, screenWidth, skbHeight,
                mContext);
        break;

      case R.xml.skb_phone:
        majorSkb = skbPool.getSoftKeyboard(R.xml.skb_phone, R.xml.skb_phone, screenWidth, skbHeight,
            mContext);
        break;
      default:
    }
  ...
  private SoftKeyboardView inKeyboardView(int x, int y, int positionInParent[]) {
    if (mPopupSkbShow) {
      if (mPopupX <= x
          && mPopupX + mPopupSkb.getWidth() > x
          && mPopupY <= y
          && mPopupY + mPopupSkb.getHeight() > y) {
  ...
  private void popupSymbols() {
    ...
    if (popupResId > 0) {
      ...
      SkbPool skbPool = SkbPool.getInstance();
      SoftKeyboard skb =
          skbPool.getSoftKeyboard(popupResId, popupResId, miniSkbWidth, miniSkbHeight, mContext);
      if (null == skb) return;
      ...
      if (null == mPopupSkbView) {
        ...
        mPopupSkbView.onMeasure(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
      }
      ...
      mPopupSkb.setWidth(
          skb.getSkbCoreWidth() + mPopupSkbView.getPaddingLeft() + mPopupSkbView.getPaddingRight());
      ...
      mPopupSkb.showAtLocation(this, Gravity.NO_GRAVITY, mPopupX, mPopupY + mXyPosTmp[1]);
      mPopupSkbShow = true;
  ...
  @Override public boolean onTouchEvent(MotionEvent event) {
    ...
    if (event.getAction() == MotionEvent.ACTION_MOVE) {
      if (Math.abs(x - mXLast) <= MOVE_TOLERANCE && Math.abs(y - mYLast) <= MOVE_TOLERANCE) {
        return true;
      }
    }
    ...
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        ...
        if (null != mSkv) {
          mSoftKeyDown =
              mSkv.onKeyPress(x - mSkvPosInContainer[0], y - mSkvPosInContainer[1], mLongPressTimer,
                  false);
        }
        break;
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (null != skv) {
          if (skv != mSkv) {
            mSkv = skv;
            mSoftKeyDown = mSkv.onKeyPress(x - mSkvPosInContainer[0], y - mSkvPosInContainer[1],
                mLongPressTimer, true);
          } else if (null != skv) {
            if (null != mSkv) {
              mSoftKeyDown = mSkv.onKeyMove(x - mSkvPosInContainer[0], y - mSkvPosInContainer[1]);
      ...
      case MotionEvent.ACTION_UP:
        ...
        if (null != mSkv) {
          mSkv.onKeyRelease(x - mSkvPosInContainer[0], y - mSkvPosInContainer[1]);
        }
  ...
  public boolean onTouch(View v, MotionEvent event) {
    ...
    MotionEvent newEv =
        MotionEvent.obtain(event.getDownTime(), event.getEventTime(), event.getAction(),
            event.getX() + mPopupX, event.getY() + mPopupY, event.getPressure(), event.getSize(),
