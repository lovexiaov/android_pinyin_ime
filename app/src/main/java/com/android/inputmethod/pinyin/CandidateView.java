...
public class CandidateView extends View {
  ...
  public boolean activeCursorForward() {
    ...
    int pageSize = mDecInfo.mPageStart.get(mPageNo + 1) - mDecInfo.mPageStart.get(mPageNo);
  ...
  @Override protected void onDraw(Canvas canvas) {
    ...
    int yPos = (getMeasuredHeight() - (mFmiCandidates.bottom - mFmiCandidates.top)) / 2
        - mFmiCandidates.top;
    ...
    for (int i = 0; i < pSize; i++) {
      ...
      if (mActiveCandInPage == i && mEnableActiveHighlight) {
        mActiveCellRect.set(xPos, mPaddingTop + 1, xPos + itemTotalWidth,
            getHeight() - mPaddingBottom - 1);
        mActiveCellDrawable.setBounds((int) mActiveCellRect.left, (int) mActiveCellRect.top,
            (int) mActiveCellRect.right, (int) mActiveCellRect.bottom);
        mActiveCellDrawable.draw(canvas);
      }

      if (mCandRects.size() < pSize) mCandRects.add(new RectF());
      mCandRects.elementAt(i)
          .set(xPos - 1, yPos + mFmiCandidates.top, xPos + itemTotalWidth + 1,
              yPos + mFmiCandidates.bottom);

      // Draw footnote
      if (mShowFootnote) {
        canvas.drawText(footnote, xPos + (candMargin - footnoteSize) / 2, yPos, mFootnotePaint);
      }

      // Left margin
      xPos += candMargin;
      if (candidateWidth > mContentWidth - xPos - centerOffset) {
        cand = getLimitedCandidateForDrawing(cand, mContentWidth - xPos - centerOffset);
      }
      if (mActiveCandInPage == i && mEnableActiveHighlight) {
  ...
  private float drawVerticalSeparator(Canvas canvas, float xPos) {
    mSeparatorDrawable.setBounds((int) xPos, mPaddingTop,
        (int) xPos + mSeparatorDrawable.getIntrinsicWidth(), getMeasuredHeight() - mPaddingBottom);
  ...
  public boolean onTouchEventReal(MotionEvent event) {
    ...
    if (null == mDecInfo || !mDecInfo.pageReady(mPageNo) || mPageNoCalculated != mPageNo) {
      return true;
    }
    ...
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_MOVE:
        clickedItemInPage = mapToItemInPage(x, y);
        if (clickedItemInPage >= 0 && (clickedItemInPage != mTimer.getActiveCandOfPageToShow()
            || mPageNo != mTimer.getPageToShow())) {
          showBalloon(clickedItemInPage, true);
          mTimer.startTimer(BalloonHint.TIME_DELAY_SHOW, mPageNo, clickedItemInPage);
        }
    }
  ...
  private void showBalloon(int candPos, boolean delayedShow) {
    ...
    mBalloonHint.setBalloonConfig(
        mDecInfo.mCandidatesList.get(mDecInfo.mPageStart.get(mPageNo) + candPos), 44, true,
        mImeCandidateColor, desired_width, desired_height);

    getLocationOnScreen(mLocationTmp);
    mHintPositionToInputView[0] =
        mLocationTmp[0] + (int) (r.left - (mBalloonHint.getWidth() - desired_width) / 2);
