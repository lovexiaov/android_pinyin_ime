...
public class XmlKeyboardLoader {
      ...
      while (mXmlEventType != XmlResourceParser.END_DOCUMENT) {
        ...
        if (mXmlEventType == XmlResourceParser.START_TAG) {
          ...
          if (XMLTAG_SKB_TEMPLATE.compareTo(attribute) == 0) {
            ...
            Drawable balloonBg = getDrawable(xrp, XMLATTR_BALLOON_BG, null);
            Drawable popupBg = getDrawable(xrp, XMLATTR_POPUP_BG, null);
            ...
            globalColorHl = getColor(xrp, XMLATTR_COLOR_HIGHLIGHT, 0xffffffff);
            globalColorBalloon = getColor(xrp, XMLATTR_COLOR_BALLOON, 0xffffffff);
            ...
            Drawable hlBg = getDrawable(xrp, XMLATTR_KEYTYPE_HLBG, null);
            int color = getColor(xrp, XMLATTR_COLOR, globalColor);
            int colorHl = getColor(xrp, XMLATTR_COLOR_HIGHLIGHT, globalColorHl);
            int colorBalloon = getColor(xrp, XMLATTR_COLOR_BALLOON, globalColorBalloon);
            ...
            if (null != icon && null != iconPopup) {
              mSkbTemplate.addDefaultKeyIcons(keyCode, icon, iconPopup);
            }
      ...
      while (mXmlEventType != XmlResourceParser.END_DOCUMENT) {
        ...
        if (mXmlEventType == XmlResourceParser.START_TAG) {
          ...
          if (XMLTAG_KEYBOARD.compareTo(attr) == 0) {
            ...
            int skbTemplateId = xrp.getAttributeResourceValue(null, XMLATTR_SKB_TEMPLATE, 0);

            // 1.2 Try to get the template from pool. If it is not
            // in, the pool will try to load it.
            mSkbTemplate = skbPool.getSkbTemplate(skbTemplateId, mContext);

            if (null == mSkbTemplate || !attrSkb.getAttributes(attrDef)) {
              return null;
            }

            boolean cacheFlag = getBoolean(xrp, XMLATTR_SKB_CACHE_FLAG, DEFAULT_SKB_CACHE_FLAG);
            boolean stickyFlag = getBoolean(xrp, XMLATTR_SKB_STICKY_FLAG, DEFAULT_SKB_STICKY_FLAG);
            boolean isQwerty = getBoolean(xrp, XMLATTR_QWERTY, false);
            boolean isQwertyUpperCase = getBoolean(xrp, XMLATTR_QWERTY_UPPERCASE, false);

            softKeyboard = new SoftKeyboard(resourceId, mSkbTemplate, mSkbWidth, mSkbHeight);
            softKeyboard.setFlags(cacheFlag, stickyFlag, isQwerty, isQwertyUpperCase);

            mKeyXMargin = getFloat(xrp, XMLATTR_KEY_XMARGIN, mSkbTemplate.getXMargin());
            mKeyYMargin = getFloat(xrp, XMLATTR_KEY_YMARGIN, mSkbTemplate.getYMargin());
  ...
  private SoftKey getSoftKey(XmlResourceParser xrp, KeyCommonAttributes attrKey)
    ...
    if (toggleKey) {
      ...
      if (!((SoftKeyToggle) softKey).setToggleStates(
          getToggleStates(attrKey, (SoftKeyToggle) softKey, keyCode))) {
        return null;
      }
    } else {
      softKey = new SoftKey();
  ...
  private SoftKeyToggle.ToggleState getToggleStates(KeyCommonAttributes attrKey,
      SoftKeyToggle softKey, int defKeyCode) throws XmlPullParserException, IOException {
    ...
    if (mXmlEventType == XmlResourceParser.START_TAG) {
      ...
      if (attr.compareTo(XMLTAG_TOGGLE_STATE) == 0) {
        SoftKeyToggle.ToggleState nextState = getToggleStates(attrKey, softKey, defKeyCode);
