...
public class SoftKeyToggle extends SoftKey {
  ...
  @Override public String getKeyLabel() {
    ToggleState state = getToggleState();
    if (null != state) return state.mKeyLabel;
    return mKeyLabel;
  }

  @Override public Drawable getKeyBg() {
    ToggleState state = getToggleState();
  ...
  @Override public Drawable getKeyHlBg() {
    ToggleState state = getToggleState();
    if (null != state && null != state.mKeyType) {
  ...
  @Override public int getColorHl() {
    ToggleState state = getToggleState();
    if (null != state && null != state.mKeyType) {
  ...
  @Override public int getColorBalloon() {
    ToggleState state = getToggleState();
    if (null != state && null != state.mKeyType) {
  ...
  @Override public boolean isKeyCodeKey() {
    ToggleState state = getToggleState();
    if (null != state) {
  ...
  @Override public boolean isUserDefKey() {
    ToggleState state = getToggleState();
    if (null != state) {
      if (state.mKeyCode < 0) return true;
      return false;
  ...
  @Override public boolean isUniStrKey() {
    ToggleState state = getToggleState();
    if (null != state) {
  ...
  @Override public boolean needBalloon() {
    ToggleState state = getToggleState();
    if (null != state) {
  ...
  @Override public boolean repeatable() {
    ToggleState state = getToggleState();
    if (null != state) {
  ...
  @Override public void changeCase(boolean lowerCase) {
    ToggleState state = getToggleState();
  ...
  private ToggleState getToggleState() {
    ...
    while ((null != state) && (state.mIdAndFlags & KEYMASK_TOGGLE_STATE) != stateId) {
